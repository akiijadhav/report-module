...
const SetPasswordPage: NextPageWithLayout = () => {
  ...
  useEffect(() => {
    ...
    async function verifyToken(Email: string, Token: string) {
      ...
      fetchWrapper({
        url: `${process.env.NEXT_PUBLIC_API_BASE_URL}/account/verify-set-password-token`,
        method: 'POST',
        body: {
          Email,
          Token,
  ...
  const submitHandler = async (values: {
    ...
    fetchWrapper({
      url: `${process.env.NEXT_PUBLIC_API_BASE_URL}/account/set-password`,
      method: 'POST',
      body: {
        Email: values.email,
        Token: token,
      ...
      <>
        <RocFormContainer>
          ...
          <h1 className={`font-semibold text-2xl text-gray-800`}>
            {t('error.setup_password')}
          </h1>
          <p className="font-normal text-base text-gray-600">
            {t('error.contact_your_admin')}
          </p>
        </RocFormContainer>
      </>
    );
        ...
        <RocFormContainer>
          ...
          <div className={`${robotoFont} text-sm text-[#626C84]`}>
            ...
            <div className="flex items-center gap-1">
              ...
              />
              {t('error.contain_atleast_8_char')}
            </div>
            <div className="flex items-center gap-1">
              <Image
                src={
              ...
              />
              {t('error.contain_lower_upper_A_Z')}
            </div>
            <div className="flex items-center gap-1">
              <Image
                src={
              ...
              />
              {t('error.contain_number_symbol_0_9')}
            </div>
          </div>
          {responseError && (
